...
#Volumen de los reservorios de 12 canales
multi_well_rack_vol = 13000
# Número de columnas que ocupan las muestras
num_cols = math.ceil(NUM_SAMPLES / 8) # Columns we are working on
# Se calculan el número de muestras reales que se van a trabajar. Se trabajan columnas completas haya o no muestra
NUM_SAMPLES = num_cols * 8
# Factor para calcular la cantidad de reactivo necesaria en función del volumen de muestra introducido
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    STEPS = { #Dictionary with STEP activation, description, and times
            ...
            2:{'Execute': False, 'description': 'Wait with magnet OFF', 'wait_time': 600}, #300
            3:{'Execute': False, 'description': 'Incubate wait with magnet ON', 'wait_time': 600}, #300
            4:{'Execute': False, 'description': 'Remove supernatant'},#
            5:{'Execute': False, 'description': 'Switch off magnet'},#
            6:{'Execute': False, 'description': 'Add VHB/WB1'},#
            7:{'Execute': False, 'description': 'Incubate wait with magnet ON', 'wait_time': 600},#300
            8:{'Execute': False, 'description': 'Remove supernatant'},#
            9:{'Execute': False, 'description': 'Switch off magnet'},#
            10:{'Execute': False, 'description': 'Add SPR/WB2'},#
            11:{'Execute': False, 'description': 'Incubate wait with magnet ON', 'wait_time': 600},#300
            12:{'Execute': False, 'description': 'Remove supernatant'},#
            13:{'Execute': False, 'description': 'Switch off magnet'},#
            14:{'Execute': False, 'description': 'Add SPR/WB2'},#
            15:{'Execute': False, 'description': 'Incubate wait with magnet ON', 'wait_time': 600},#300
            16:{'Execute': False, 'description': 'Remove supernatant'},#
            17:{'Execute': False, 'description': 'Allow to dry', 'wait_time': 600},#900
            18:{'Execute': False, 'description': 'Switch off magnet'},#
            19:{'Execute': False, 'description': 'Add water'},#
            20:{'Execute': False, 'description': 'Wait with magnet OFF', 'wait_time': 600},#600
            21:{'Execute': False, 'description': 'Incubate wait with magnet ON', 'wait_time': 600},#300
    ...
    Lysis = Reagent(name = 'Lysis',
                    ...
                    num_channels = 8,
                    reagent_volume = 265 * reagent_proportion, # reagent volume needed per sample. 200ul of sample needs 265ul of lysis
    ...
    SPR = Reagent(name = 'SPR',
                    ...
                    flow_rate_aspirate = 1.5, # Original = 1
    ...
    Elution = Reagent(name = 'Elution',
                    ...
                    flow_rate_aspirate = 1.2, # Original 0.5
    ...
    def custom_mix(pipet, reagent, location, vol, rounds, blow_out, offset, mix_height = 1):
        '''
    ...
    def calc_height(reagent, cross_section_area, aspirate_volume):
        ...
        height = (( (reagent.vol_well[reagent.col] - aspirate_volume - (reagent.v_cono)) / cross_section_area ) + reagent.h_cono) - 2.5
    ...
    def move_vol_multi(pipet, reagent, source, dest, vol, x_offset_source, x_offset_dest, pickup_height, wait_time, blow_out, lifting = False,  intermidiate_pos = None, dispense_before = 0):
        ...
        if dispense_before:
            pipet.dispense(dispense_before, source.top(z = 0))
        ...
        if wait_time != 0:
            for i in range (3):
                if lifting:
                    pipet.move_to(s.move(Point( z = 0.1)))
                    ctx.comment("Levantando 0.1")
                ctx.delay(seconds=wait_time/3, msg='Waiting for ' + str(wait_time) + ' seconds.')
            if lifting:
                pipet.move_to(s.move(Point( z = 0.1)))
                ctx.comment("Levantando 0.2")
                ctx.delay(seconds = 1, msg='Waiting for ' + str(1) + ' seconds.')
        elif lifting:
            pipet.move_to(s.move(Point( z = 0.3)))
            ctx.comment("Levantando 0.3")
        ...
        if wait_time != 0:
            ctx.delay(seconds= wait_time/2, msg='Waiting for ' + str(wait_time/2) + ' seconds.')
...
###############################################################################
    ...
    #Transfer lysis
        ...
        for i in range(num_cols):
            ...
            if not m300.hw_pipette['has_tip']:
                if find_next_tip(m300.tip_racks) is None:
                    #if not ctx.is_simulating():
                        #subprocess.call('mpg123 ./var/lib/jupyter/notebooks/sonido_alarma_submarino.mp3', shell=True)
                    ctx.pause('Reemplace los tipracks en los slots 6 y 7. Después pulse resume.')
                    ctx.comment("Tip_racks renovados")
                    m300.reset_tipracks()
                m300.pick_up_tip()
            ...
            custom_mix(m300, Lysis, location = work_destinations[i], vol = 150,rounds = 10, blow_out = False, mix_height = 1, offset = 0)
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_transfer_vol:
                ...
                move_vol_multi(m300, reagent = Elution, source = work_destinations[i],
                               ...
                               pickup_height = pickup_height,  wait_time = 2, blow_out = False, lifting = True, dispense_before = 10)
        ...
        for i in range(num_cols):
            ...
            if not m300.hw_pipette['has_tip']:
                if find_next_tip(m300.tip_racks) is None:
                    ...
                    ctx.comment("Tip_racks renovados")
                    m300.reset_tipracks()
                    if reuse_tip_list[-1] is None:
                        reuse_tip_list[-1] = find_next_tip(m300.tip_racks)
                if i != 0:
            ...
            for j,transfer_vol in enumerate(vhb_transfer_vol):
                ...
                dest = work_destinations[i], vol = transfer_vol, intermidiate_pos = inter_pos, x_offset_source = x_offset_source, x_offset_dest = x_offset_dest,
                pickup_height = pickup_height, wait_time = 1, blow_out = False)
                
                if ( j < (vhb_trips - 1) ):
                    m300.move_to(inter_pos)

            custom_mix(m300, VHB, location = work_destinations[i], vol = 150, rounds = 8, blow_out = False, mix_height = 2, offset = x_offset_dest - 1)
            m300.move_to(work_destinations[i].top(0))
            m300.air_gap(20)
            m300.return_tip(home_after = False)
            
        #Reseteamos las puntas usadas para reciclarlas en el lavado
        reset_tipWell_OnTipTracker (reuse_tip_list, m300.channels)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            custom_mix(m300, SPR, location = work_destinations[i], vol = 150,
                rounds = 8, blow_out = False, mix_height = 2, offset = x_offset_dest)
            m300.move_to(work_destinations[i].top(0))
            m300.air_gap(100)
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_transfer_vol:
                ...
                move_vol_multi(m300, reagent = VHB, source = work_destinations[i],
                                dest = waste, vol = transfer_vol, x_offset_source = x_offset_source, x_offset_dest = x_offset_dest,
                                pickup_height = pickup_height, wait_time = 3, blow_out = False, lifting = True, dispense_before = 10)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            custom_mix(m300, SPR, location = work_destinations[i], vol = 150, rounds = 8, blow_out = False, mix_height = 2, offset = x_offset_dest)
            m300.move_to(work_destinations[i].top(0))
            m300.air_gap(20)
            m300.return_tip(home_after = False)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_transfer_vol:
                #Pickup_height is fixed here
                pickup_height = 0.4 # Original 0.5
                ctx.comment('Aspirate from deep well column: ' + str(i+1))
                ctx.comment('Pickup height is ' + str(pickup_height) +' (fixed)')
                m300.air_gap(10)
        ...
        for i in range(num_cols):
            ...
            move_vol_multi(m300, reagent = SPR, source = work_destinations[i],
                           ...
                           pickup_height = pickup_height, wait_time = 2, blow_out = False, lifting = True)
            m300.air_gap(100)
            m300.return_tip()
            tip_track['counts'][m300] += 8
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in water_wash_vol:
                ...
                pickup_height = pickup_height,  wait_time = 0, blow_out = False)

            ctx.comment(' ')
            ctx.comment('Mixing sample with Water')
            #Mixing
            custom_mix(m300, Elution, work_destinations[i], vol = 40, rounds = 8, blow_out = False, mix_height = 2, offset = x_offset_dest)
            m300.move_to(work_destinations[i].top(0))
            m300.air_gap(100)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in elution_vol:
                ...
                pickup_height = pickup_height, wait_time = 2, blow_out = True, lifting = True)
            m300.air_gap(100)
            m300.drop_tip(home_after = False)
            #Aumenta la cuenta de puntas usadas
            tip_track['counts'][m300] += 8
...
###############################################################################
    ...
    ctx.comment('Terminado! \nMueva la placa qPCR del modulo de temperatura a la estación C para la preparación de PCR.')
    ctx.comment('Used tips in total: '+str(tip_track['counts'][m300]))
...
#run(ctx)
''' CHANGELOG
     v1.0.1: Con respecto a la versión v1.0 se soluciona el problema de el error cuando se acaban las puntas y se recarga 
             los tipracks. La lista de reutilizacion se quedaba con un noneobject ya que esta lista se generaba antes de recargar los tipracks.
             Esto ha implicado quitar de pick_up el control de fin de puntas con la excepción pertinente.
     v1.0.2: - Se incluye en move_vol_multi pequeños movimientos hacia arriba siempre que se meta una espera. Así se asegura de que la punta no esté ocluida por el fondo
             - velocidades de aspiración de Binding, elución y SPR aumentadas
             - Se incluyen diversas aspiraciones para evitar gotas
             - Se controla en todo momento si se terminan las puntas y se le pide al usuario que las reemplace con un sonido de alerta previo
             - Se elimina el bloque del final que apagaba las luces
