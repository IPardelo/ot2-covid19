...
# Load specific run parameters from run_config.json created by custom GUI
#filepath = "/root/ot2-covid19/library/protocols/run_config.json"
...
# Protocol parameters
# ------------------------
numero_muestras = int(cont["muestras"])                         # Número de muestras (sin las muestras de control)
                                            # (máximo 94 si es una mastermix, o 44 si es doble)                                            
brand_name = cont["kit"]
tipo_de_tubo = cont["tubo"]                  # Tipo de tubo que contiene el ARN: 'labturbo' o 'criotubo'
...
#    source_master_mix = ctx.load_labware('opentrons_24_aluminumblock_generic_2ml_screwcap', '7', 'Bloque Aluminio opentrons 24 screwcaps 2000 µL')
    source_master_mix = ctx.load_labware('opentrons_24_aluminumblock_nest_1.5ml_snapcap', '7', 'Bloque Aluminio opentrons 24 eppendorf 1500 µL')
...
#        'opentrons_24_tuberack_generic_2ml_screwcap', slot,
        'opentrons_24_tuberack_nest_1.5ml_snapcap', slot,
#        'source tuberack with screwcap' + str(i + 1)) for i, slot in enumerate(['5', '6', '2', '3'][:rack_num])
        'source tuberack with snapcap' + str(i + 1)) for i, slot in enumerate(['5', '6', '2', '3'][:rack_num])
    ])
